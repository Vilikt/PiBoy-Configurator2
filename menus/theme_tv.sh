...
function update_variables() {
	TV_THEME=`python $PIBOYCONF_ROOT_FOLDER/readers/piboy_config_file.py theme_tv get`
}

function manage_choice() {
	export choice
	python $PIBOYCONF_ROOT_FOLDER/readers/piboy_config_file.py theme_tv set
	update_variables
}
...
function main_menu() {
    ...
    while true; do
        choice=$(dialog --backtitle "PiBoy Configurator v.$piboyconf_version" --title "Select ES Theme on TV mode" \
            --ok-label "Select" --cancel-label "Back" --no-tags \
            --menu "Actual is '$TV_THEME'" 25 75 20 "${MENU_LINES[@]}" 2>&1 > /dev/tty)
